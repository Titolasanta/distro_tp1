...
def parse_config_params():
	...
	try:
		config_params["monitor_ip"] = os.environ["MONITOR_IP"]
		config_params["monitor_port"] = int(os.environ["MONITOR_PORT"])
		config_params["port"] = int(os.environ["PORT"])
		config_params["cord_ip"] = os.environ["COR_IP"]
		config_params["cord_port"] = os.environ["COR_PORT"]

		config_params["ID"] = os.environ["SL_ID"]
...
def main():	
	...
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
		...
		s.sendall(("sl"+config_params["ID"]).encode('utf-8'))
		s.close()

	# Initialize server and start server loop
	server = Server(config_params["port"],config_params["monitor_ip"],config_params["monitor_port"])
